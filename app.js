...
function evaluateSpeakingIELTS(userSpeech, correctText) {
    ...
    function playSound(soundType) {
        ...
        // Điều hướng các cuộc gọi cũ sang SoundEngine mới
        switch(soundType) {
            case sounds.correct: SoundEngine.playCorrect(); break;
            case sounds.incorrect: SoundEngine.playIncorrect(); break;
            case sounds.start: SoundEngine.playStart(); break;
            case sounds.victory: SoundEngine.playVictory(); break;
        }
    }
...
// --- NÂNG CẤP: AUDIO ENGINE (WEB AUDIO API) ---
    // --- NÂNG CẤP: SOUND ENGINE V2 (GAMIFIED ARCADE EDITION) ---
    const SoundEngine = {
        ctx: null,
        init() {
            if (!this.ctx) {
        ...
        // Hàm lõi tạo âm thanh siêu cấp với hiệu ứng nảy (Attack) và trượt cao độ (Slide)
        playTone({ freq, type = 'sine', duration = 0.1, vol = 0.1, slideTo = null, delay = 0 }) {
            try {
                this.init();
                ...
                // Cài đặt tần số (cao độ)
                osc.frequency.setValueAtTime(freq, startTime);
                if (slideTo) {
                    // Hiệu ứng game: Trượt âm thanh lên/xuống (Pitch Bend)
                    osc.frequency.exponentialRampToValueAtTime(slideTo, startTime + duration);
                }
                
                // Hiệu ứng Game: Cắt âm gọn gàng tạo độ "nảy" (ADSR Envelope)
                gain.gain.setValueAtTime(0, startTime);
                gain.gain.linearRampToValueAtTime(vol, startTime + 0.02); // Tấn công nhanh
                gain.gain.exponentialRampToValueAtTime(0.001, startTime + duration); // Tắt mượt
        ...
        // 1. ÂM THANH ĐÚNG: Tiếng "Ting Ting" ăn tiền/hạt dẻ cực bắt tai (Kiểu Super Mario)
        playCorrect() {
            // Sóng Square tạo cảm giác 8-bit, Sóng Sine tạo độ trong trẻo
            this.playTone({ freq: 987.77, duration: 0.1, vol: 0.15, type: 'square' });
            this.playTone({ freq: 1318.51, duration: 0.3, vol: 0.15, type: 'sine', delay: 0.08 });
        },

        // 2. ÂM THANH SAI: Tiếng "Bloop" trượt dần xuống (Cảm giác rơi rớt/xịt)
        playIncorrect() {
            this.playTone({ freq: 300, slideTo: 100, duration: 0.35, vol: 0.2, type: 'sawtooth' });
            this.playTone({ freq: 250, slideTo: 80, duration: 0.4, vol: 0.2, type: 'triangle', delay: 0.05 });
        },

        // 3. ÂM THANH BẮT ĐẦU: Nhịp điệu "Level Up" dồn dập
        playStart() {
            const notes = [523.25, 659.25, 783.99, 1046.50]; // Các nốt C5, E5, G5, C6
            notes.forEach((freq, i) => {
                this.playTone({ freq: freq, duration: 0.15, vol: 0.12, type: 'square', delay: i * 0.08 });
            });
        },

        // 4. ÂM THANH CHIẾN THẮNG: Fanfare (Kèn đồng) hoành tráng bùng nổ
        playVictory() {
            // Dải âm thanh lướt nhanh lên cao
            const sweep = [523.25, 587.33, 659.25, 698.46, 783.99, 880.00, 987.77];
            sweep.forEach((freq, i) => {
                this.playTone({ freq: freq, duration: 0.1, vol: 0.1, type: 'triangle', delay: i * 0.05 });
            });
            // Hợp âm chốt cuối cùng (C6 + E6 + G6) vang rất dài và sáng
            const chordTime = sweep.length * 0.05;
            this.playTone({ freq: 1046.50, duration: 1.0, vol: 0.15, type: 'sine', delay: chordTime }); // C6
            this.playTone({ freq: 1318.51, duration: 1.0, vol: 0.12, type: 'sine', delay: chordTime }); // E6
            this.playTone({ freq: 1567.98, duration: 1.0, vol: 0.1,  type: 'sine', delay: chordTime }); // G6
        }
    };
...
async function startIELTSSpeaking(question) {
    ...
    function renderOrdering(question) {
     ...
     itemsToShow.forEach(itemText => {
            ...
            // --- NÂNG CẤP: XÓA TIỀN TỐ (1), (2), 1., 1/ ĐỂ CHỐNG GỢI Ý ---
            // Regex này sẽ xóa các mẫu như (1), 1., 1/ ở đầu câu
            const displayText = itemText.replace(/^(\(\d+\)|\d+[\.\/])\s*/, '');
            
            // Lưu ý: data-text vẫn giữ safeItemText (có số) để so sánh với đáp án gốc
            itemsHTML += `<li class="ordering-item" draggable="true" data-text="${safeItemText}">${displayText}</li>`;
        });
