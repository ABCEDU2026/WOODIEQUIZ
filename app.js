...
function startQuiz(mode) {
        ...
        if ($('app-header')) { $('app-header').textContent = title; $('app-header').classList.remove('hidden'); }

        // =========================================================================
        // [A90 ELITE ENGINE] THUẬT TOÁN ĐIỀU HƯỚNG NHẬN THỨC
        // =========================================================================
        let pool = [];
        
        if (mode === 'retry_wrong') {
            // Bước 1: Quét lịch sử thất bại trong LocalStorage
            const currentUser = state.allUsersData[state.currentUserName];
            const logs = currentUser ? (currentUser.logs || []) : [];
            
            // Bước 2: Chỉ cho phép làm lại nếu bài test trước đó có tỉ lệ đúng < 90%
            const badSessions = logs.filter(l => l.topic === title && parseInt(l.acc) < 90);
            
            if (badSessions.length === 0) {
                showAlert(`✨ Tuyệt vời! Hệ thống xác nhận không có lỗ hổng kiến thức nghiêm trọng nào trong chuyên đề này.`);
                return;
            }
            
            // Bước 3: Trong tương lai, có thể nâng cấp JSON để cắm cờ (flag) từng câu hỏi sai.
            // Hiện tại, ép học sinh làm lại bộ đề trộn ngẫu nhiên của chủ đề yếu kém.
            pool = shuffleArray(bank).slice(0, 15); 
            title = `🔁 VÁ LỖ HỔNG: ${title.split(' - ')[1] || title}`;
            
        } else if (mode === 'smart_review') {
            // Tư duy Ivy League: Trộn 30% lý thuyết nền tảng + 70% câu hỏi vận dụng cao
            // Yêu cầu file JSON phải có trường "level": "hard"
            const hardQuestions = bank.filter(q => q.level === 'hard');
            const normalQuestions = bank.filter(q => q.level !== 'hard');
            
            const selectedHard = shuffleArray(hardQuestions).slice(0, 7);
            const selectedNormal = shuffleArray(normalQuestions).slice(0, 3);
            
            pool = shuffleArray([...selectedHard, ...selectedNormal]);
            if (pool.length === 0) pool = shuffleArray(bank).slice(0, 10); // Fallback
            title = `🧠 ÔN THÔNG MINH: ${title.split(' - ')[1] || title}`;
            
        } else if (mode === 'random') {
            pool = shuffleArray(bank).slice(0, 20);
        } else {
            pool = [...bank];
        }

        // Render UI tiếp tục như bình thường...
        ...
        if (!banner) { banner = document.createElement('div'); banner.id = 'quiz-topic-banner'; banner.className = 'quiz-topic-banner'; const sidebar = document.querySelector('.sidebar'); if (sidebar) sidebar.insertBefore(banner, sidebar.firstChild); }
...
function renderMultiResponse(q) {
    // 1. Tạo giao diện và dùng chuẩn document.getElementById để tránh lỗi ReferenceError
    ...
    const optsContainer = document.getElementById('multi-opts');
    
    // 2. Tạo nút Kiểm tra
    const chk = document.createElement('button'); 
    chk.id = 'multi-check-btn'; 
    chk.className = 'nav-btn'; 
    chk.textContent = 'Kiểm tra'; 
    chk.style.marginTop = "15px";

    // 3. Duyệt và Render TỪNG ĐÁP ÁN
    Object.keys(q.options || {}).sort().forEach(k => { 
        
        // Dùng (e) => {} để đảm bảo không bị mất context "this" khi click
        const toggleSelection = (e) => {
            const btn = e.currentTarget;
            btn.classList.toggle('selected');
            // Nút Kiểm tra chỉ sáng lên khi có ít nhất 1 đáp án được chọn
            chk.disabled = optsContainer.querySelectorAll('.selected').length === 0;
        };

        // Kế thừa hàm createOptionButton mặc định trong hệ thống của bạn
        const b = createOptionButton(k, q.options[k], toggleSelection, q.isAnswered || state.isReviewMode); 
        
        // Đưa nút ra màn hình thực tế
        optsContainer.appendChild(b);

        // 4. Xử lý logic nếu đang ở chế độ Xem Lại (Review Mode) hoặc đã trả lời
        if (q.isAnswered || state.isReviewMode) {
            ...
            if (expArea && expText) {
                ...
                expText.innerHTML = q.explanation || '<span style="color: #666; font-style: italic;">Không có giải thích chi tiết.</span>';
            }
            
            // Khôi phục đáp án học sinh đã chọn
            if (q.userAnswer && q.userAnswer.includes(k)) {
                b.classList.add('selected');
            }
        }
    });
    
    // 5. Kích hoạt nút Kiểm tra vào DOM
    if (!q.isAnswered && !state.isReviewMode) { 
        chk.disabled = true; 
        chk.onclick = () => checkMultiResponseAnswer(); 
   ...
   function bindCoreEvents() {
        // [A90 ELITE] ĐÃ SỬA ĐÚNG LUỒNG CHO 4 NÚT CHIẾN LƯỢC
        $('start-random-btn')?.addEventListener('click', () => startQuiz('random'));
        $('start-full-btn')?.addEventListener('click', () => startQuiz('full'));
        $('start-smart-btn')?.addEventListener('click', () => startQuiz('smart_review'));
        $('retry-wrong-btn')?.addEventListener('click', () => startQuiz('retry_wrong'));
...
// [A90 ELITE] CHẠY TRỰC TIẾP TRÍ TUỆ NHÂN TẠO GEMINI (KHÔNG QUA TRUNG GIAN)
        document.getElementById('ask-ai-btn')?.addEventListener('click', async () => {
            const q = state.activeQuestions[state.currentQuestionIndex];
            if (!q) return;
            
            const aiArea = document.getElementById('ai-response-area') || document.querySelector('.ai-response-area');
            const aiText = document.getElementById('ai-content-text');
            const aiLoading = document.getElementById('ai-loading-text');
            
            if (aiArea) aiArea.classList.remove('hidden');
            if (aiLoading) aiLoading.style.display = 'block';
            if (aiText) aiText.innerHTML = '';
            
            // CHÚ ĐỨC LƯU Ý: Chú phải tạo 1 API Key miễn phí tại Google AI Studio và dán vào giữa 2 dấu ngoặc kép bên dưới
            const GEMINI_API_KEY = "AIzaSyAxI6RIkJfeqSSuHlA3zP9J-IWI4yANn8A"; 
            
            if (GEMINI_API_KEY === "AIzaSyAxI6RIkJfeqSSuHlA3zP9J-IWI4yANn8A") {
                if (aiLoading) aiLoading.style.display = 'none';
                if (aiText) aiText.innerHTML = '<span style="color:red;font-weight:bold;">🚨 THIẾU CHÌA KHÓA:</span> Chú Đức chưa điền Gemini API Key vào code.';
                return;
            }

            const geminiUrl = "https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=" + GEMINI_API_KEY;
            
            const prompt = `You are an Ivy League STEM & Elite Finance Mentor. Explain the deepest logical framework of this question to a student.\n\nQuestion: ${q.question}\nCorrect Answer: ${q.answer}\nContext/Explanation if any: ${q.explanation || 'None'}\n\nProvide a concise, highly analytical explanation.`;
            ...
            try {
                ...
                if (data.candidates && data.candidates[0].content.parts[0].text) {
                    // Hiển thị câu trả lời siêu trí tuệ lên màn hình
                 if (aiText) aiText.innerHTML = data.candidates[0].content.parts[0].text.replace(/\n/g, '<br>');
                    if (aiText) aiText.innerHTML = 'AI đã nhận lệnh nhưng không thể tạo câu trả lời.';
                }
                
            } catch (err) {
                if (aiLoading) aiLoading.style.display = 'none';
                if (aiText) aiText.innerHTML = `<span style="color:red;font-weight:bold;">🚨 LỖI MẠNG:</span> ${err.message}`;
            }
        });
